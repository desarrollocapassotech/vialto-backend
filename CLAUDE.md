...
- **Pagos al transportista con liquidación vigente — resumen real en vez de fieldset editable** (`ViajeEditModal.tsx`, ago 2026): el "Acordado/Pagado/Saldo" en vivo de `PagosTransportistaFieldset` (`calcularSaldoTransportistaDesdeDraft`) siempre calculó sobre el precio plano del draft, sin mirar `liquidacionesViaje` — y `pagosTransportista` está en `CAMPOS_FISCALES_VIAJE`, así que editar filas ahí ya fallaba igual al guardar una vez que el viaje tiene liquidación vigente. La sección "Pagos al transportista" de `ViajeEditModal.tsx` ahora bifurca por `liquidacionVigente` (mismo booleano que bloquea el checkbox de IVA, arriba): si hay liquidación vigente, en vez del fieldset editable muestra `PagosTransportistaSummary` (solo lectura, `calcularSaldoTransportista` con la Liquidación real) + el badge `ViajeLiquidacionIndicador` reusado tal cual de la grilla de Viajes (fetch-on-click, abre `LiquidacionViewModal` completo — comisión, IVA, conceptos, líquido, CAE). Sin liquidación vigente, sigue el fieldset editable de siempre — ahí el draft y el real coinciden, no hace falta el resumen.
  - `PagosTransportistaSummary.onRegistrarPago` pasó a opcional: si el host no lo pasa, el botón "+ Registrar pago" no se muestra. `ViajeEditModal` recibe `onRegistrarPago?: () => void` por prop, wireado en `ViajesTenantPage.tsx` (que ya tenía `RegistrarPagoTransportistaModal` montado a nivel página) contra `viajeEditor.viajeSnapshot`; `TenantHomePage.tsx` no lo pasa (no tiene ese modal montado) — ahí el resumen queda de solo lectura sin el botón, no se agregó el modal completo solo para esto.
  - `useViajeEditor.ts` expone `patchViajeSnapshot(updated: Viaje)` — sin esto, registrar un pago mientras el editor sigue abierto para el mismo viaje dejaba `viajeSnapshot` desactualizado (mismo problema que ya se resolvía para `draft.pagosTransportista`, pero el snapshot no tenía equivalente). Se llama junto a `setDraft(...)` en el `onSuccess` de `RegistrarPagoTransportistaModal`.
