...
model Viaje {
  ...
  precioTransportistaIncluyeIva    Boolean   @default(false) // ver "Precio transportista con IVA incluido" más abajo
...
#### `precioTransportistaExterno` con IVA incluido — ago 2026

**Problema de negocio**: algunos transportistas cobran el monto TOTAL con IVA ya incluido (ej. $121 sobre un flete de $100 + 21% IVA). Si el viaje se carga con el precio sin IVA ($100) y después se registra un pago por el monto real pagado ($121), el pago supera el "acordado" del sistema y queda un saldo negativo. `Viaje.precioTransportistaIncluyeIva` (boolean, default `false`) resuelve esto: cuando está en `true`, `precioTransportistaExterno` **ya es el total con IVA**, y el sistema lo usa tal cual como acordado/monto a pagar, sin sumarle IVA de ningún otro lado.

- **Cálculo del acordado** (`calcularAcordado` en `viajes.service.ts`, espejado en el frontend por `totalPagadoTransportistaEnMonedaAcordada` de `lib/viajesTransportistaPagos.ts`): si el flag está en `true`, devuelve `precioTransportistaExterno` directamente y **corta antes** de mirar `liquidacionesViaje` — no aplica el IVA que normalmente se suma cuando hay una `Liquidacion` vinculada (`netoGravado = bruto - comisión; + IVA`). Si se agrega un cálculo nuevo de "cuánto se le debe al transportista", replicar este mismo guard al principio, antes de tocar liquidaciones.
- **Aplica a ambos modos de carga**: precio simple (`precioTransportistaExterno`) y modo desglose (`cantidadTransportista × precioUnitarioTransportista`, usado por NyM/granel) — el checkbox persiste en el mismo campo booleano sea cual sea el modo.
- **Exclusión mutua con Liquidación ARCA/CVLP (regla dura, no opcional)**: el motor de liquidación (`liquidaciones-arca/liquidaciones.service.ts#createLiquidacion`, único punto de entrada — cubre tenant, superadmin/platform e import wizard) **rechaza con `BadRequestException`** cualquier intento de incluir en una Liquidación un viaje con `precioTransportistaIncluyeIva = true`, listando los números de viaje afectados. Motivo: el CVLP calcula y discrimina el IVA por su cuenta (`arca-iva.util.ts`: `netoGravado = bruto - comisión`, siempre trata `bruto` como neto sin IVA) — liquidar un viaje "ya con IVA" duplicaría el IVA en el comprobante fiscal real autorizado por AFIP. **No se intentó "netear" el precio del lado de la Liquidación** (restarle el IVA antes de sumarlo al bruto) porque ensuciaría `LiquidacionViaje.subtotal` (el detalle "tarifa × tn" que se le muestra al transportista) y agrega superficie de bugs fiscales para un caso de uso (pago informal sin CVLP) que hoy no se cruza con NyM en la práctica — si a futuro hace falta que convivan, hay que revisar esta decisión, no asumir que alcanza con sumar un `if`.
- **Lock angosto, distinto al resto de `CAMPOS_FISCALES_VIAJE`**: `precioTransportistaIncluyeIva` **NO** está en `CAMPOS_FISCALES_VIAJE` — tiene su propio guard en `update()`, bloqueado **solo** por `liquidacionEstado` (no por `facturacionEstado`). Es intencional: facturación al cliente y liquidación al transportista son ejes independientes, y bloquear este flag por una factura al cliente (sin relación con el transportista) dejaría el campo atascado para siempre en viajes viejos de un tenant que recién adopta `integracion-arca` — la mayoría de esos viajes ya van a estar facturados al cliente para cuando el tenant arranque con ARCA. Si se agrega un campo nuevo relacionado solo al transportista (no al cliente), evaluar si también necesita este lock angosto en vez de sumarlo sin más a `CAMPOS_FISCALES_VIAJE`.
- **Frontend, patrón "advertir, no ocultar"** (`ViajeCreatePage.tsx`, `ViajeEditModal.tsx`): el checkbox "Incluir IVA" **siempre está visible y editable** (salvo el lock angosto de arriba, reflejado con `disabled` + `liquidacionPermiteVincular`) — cuando el tenant tiene `integracion-arca`, se muestra una advertencia (`<p className="text-xs text-amber-800/90">`) explicando la incompatibilidad, en vez de esconder el control. **No ocultar el checkbox según módulos del tenant**: la primera versión de este feature lo ocultaba con `!hasIntegracionArca &&`, y eso dejaba sin forma de destildar el flag a un viaje viejo cuyo tenant adoptó ARCA después de crearlo — bug real encontrado y corregido en la misma tanda (ago 2026). El backend (`liquidaciones.service.ts`) es la única fuente de verdad del bloqueo; el frontend solo informa.
  - `ViajeEditModal.tsx` recibe `hasIntegracionArca?: boolean` por prop (no lo resuelve solo) — cableado desde `ViajesTenantPage.tsx` (`hasLiquidacionesArca`) y `TenantHomePage.tsx` (`hasArca`), ambos ya lo calculaban para otra UI. **Limitación conocida, no resuelta a propósito**: en vista superadmin (`platform=true`, editando un tenant elegido por `?tenantId=`), `hasIntegracionArca` llega siempre en `false` porque `hasLiquidacionesArca` está definido como `!platform && canAccessIntegracionArca(...)` — así que superadmin no ve el cartel de advertencia aunque el tenant elegido sí tenga ARCA. El bloqueo real (backend) sigue funcionando igual; solo falta el cartel informativo ahí. No se resolvió porque requeriría traer los módulos del tenant elegido en modo platform solo para este texto.
- **Migración**: `prisma/migrations/20260818120000_viaje_precio_transportista_incluye_iva` (`ALTER TABLE ADD COLUMN ... BOOLEAN NOT NULL DEFAULT false`, aditiva).
- **Deuda conocida, pendiente de decisión (no implementada)**: el "Acordado/Pagado/Saldo" que se ve **en vivo mientras se edita** un viaje (`PagosTransportistaFieldset` vía `calcularSaldoTransportistaDesdeDraft`, usado tanto en `ViajeCreatePage.tsx` como en `ViajeEditModal.tsx`) siempre calcula sobre el precio plano del draft — nunca mira `liquidacionesViaje`, con o sin el flag de IVA. Es preexistente a este feature (no es un bug nuevo), pero significa que si un viaje ya tiene una Liquidación vinculada, el saldo que se ve **mientras se edita** puede no coincidir con el que se ve en la vista de solo lectura (`PagosTransportistaSummary`, que sí usa `calcularSaldoTransportista` con la Liquidación real). Antes de tocar esto, ver las opciones evaluadas — no resolver con un parche ad hoc.
...
*Última actualización: agosto 2026 (`Viaje.precioTransportistaIncluyeIva` — precio transportista con IVA incluido, exclusión mutua con Liquidación ARCA/CVLP en `liquidaciones.service.ts#createLiquidacion`, y lock angosto por `liquidacionEstado` distinto del resto de `CAMPOS_FISCALES_VIAJE`; y, de una pasada anterior, motor de importaciones — sugerencia IA con fallback Groq, `warnIfEmpty`/CUIT-país opcional, `InsertResult` con desglose creados/actualizados, dedup de Factura por `nroFactura`, split de patente compuesta en Vehículos, diff antes/después de Viajes, endpoint `tenant-tiene-datos`; y, de una pasada anterior a esa, rediseño de estados de Viaje en 3 indicadores independientes — etapa/facturación/liquidación —, split de estado de Factura en ciclo de vida + cobrado/vencida, y `Factura.ambiente`)*
