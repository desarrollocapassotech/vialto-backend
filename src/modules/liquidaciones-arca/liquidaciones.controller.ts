...
// ── RUTAS SUPERADMIN (VISTA PLATAFORMA) ─────────────────────────────────────

@ApiTags("Admin — Platform (Liquidaciones)")
@ApiBearerAuth("clerk-jwt")
@Controller("platform/integracion-arca")
@UseGuards(ClerkAuthGuard, RolesGuard)
@Roles("superadmin")
export class LiquidacionesPlatformController {
  constructor(
    private readonly service: LiquidacionesService,
    private readonly pdfService: LiquidacionPdfService,
  ) {}
  ...
  private requiredTenantId(tenantId?: string): string {
    ...
    if (!id)
      throw new BadRequestException(
        "tenantId es requerido para vistas de plataforma",
      );
    return id;
  }

  @Get("config")
  getConfig(@Query("tenantId") tenantId: string | undefined) {
  ...
  findAll(
    ...
    const id = this.requiredTenantId(tenantId);
    return this.service.findAll(id, estado);
  }
  ...
  findOne(
    ...
    return this.service.findById(tid, id);
  }
  ...
  createLiquidacion(
    ...
    const tid = this.requiredTenantId(tenantId);
    return this.service.createLiquidacion(tid, auth.userId, dto);
  }
  ...
  updateLiquidacion(
    ...
    const tid = this.requiredTenantId(tenantId);
    return this.service.updateLiquidacion(tid, id, dto);
  }
  ...
  deleteLiquidacion(
    ...
    const tid = this.requiredTenantId(tenantId);
    return this.service.deleteLiquidacion(tid, id);
  }
