...
model Viaje {
  ...
  precioTransportistaIvaIncluidoPct Float    @default(0) // % ya incluido; ver "precioTransportistaExterno con IVA incluido" más abajo
...
#### `precioTransportistaExterno` con IVA incluido — ago 2026 (rediseñado de boolean a % con neteo)

**Problema de negocio**: algunos transportistas cobran el monto TOTAL con IVA ya incluido (ej. $121 sobre un flete de $100 + 21% IVA). `Viaje.precioTransportistaIvaIncluidoPct` (`Float @default(0)`) resuelve esto: 0 = no incluye IVA (comportamiento de siempre); >0 = % de IVA ya incluido en `precioTransportistaExterno` (ej. `21`).

**Primera versión (boolean, descartada) y por qué se cambió**: la v1 era un checkbox que excluía mutuamente al viaje de cualquier Liquidación ARCA/CVLP — un viaje con el flag no se podía liquidar, punto. Probando en QA con un caso real se encontraron dos problemas: (1) el panel de resumen de `CrearLiquidacionManualModal.tsx` no miraba el flag para nada y mostraba el IVA sumado igual (bug de UI, aunque el backend sí bloqueaba la creación real — confirmado con la DB, no llegaba a persistirse ninguna `Liquidacion`); y (2) el usuario directamente no quería bloquear la operación, quería que el viaje **se pudiera liquidar igual**. Se rediseñó a un % + neteo.

**Fórmula de neteo** (`netearIvaIncluido(precioRaw, pct)` — duplicada a propósito en `viajes.service.ts` (backend) y `lib/viajesTransportistaPagos.ts` (frontend, exportada), mismo patrón de espejo que el resto de esta lógica):
```ts
function netearIvaIncluido(precioRaw: number, pctIncluido: number): number {
  if (!pctIncluido || pctIncluido <= 0) return precioRaw;
  return round2(precioRaw / (1 + pctIncluido / 100));
}
```
Ejemplo real: $121 con 21% incluido → neto $100. Con comisión 8%: comisión $8, neto gravado $92, IVA 21% = $19.32, líquido $111.32 — coherente, sin duplicar IVA, y la comisión se cobra sobre el valor neto del servicio (no sobre el IVA ya cobrado).

**Dónde se aplica el neteo (y dónde NO hace falta tocar nada)**:
- **`liquidaciones-arca/liquidaciones.service.ts#createLiquidacion`**: al armar `bruto` desde los viajes seleccionados, cada viaje se netea con su propio `precioTransportistaIvaIncluidoPct` **antes** de sumarse al agregado `bruto`. **`LiquidacionViaje.subtotal` (el detalle "tarifa × tn"/precio que ve el transportista) queda con el precio crudo tal cual se cargó, sin netear** — solo el agregado `bruto` usa el valor neteado.
- **`viajes.service.ts#calcularAcordado`** (reconstruye "cuánto se le debe al transportista" a partir de una Liquidación ya existente, para saldo pendiente/validación de pagos — espejado en frontend por `totalPagadoTransportistaEnMonedaAcordada` de `lib/viajesTransportistaPagos.ts`): igual, netea `brutoViaje` con el % del viaje antes de calcular comisión/IVA de la reconstrucción. De paso se corrigió un bug preexistente (no relacionado al feature): el código leía `lv.monto`, un campo que no existe en `LiquidacionViaje` (el campo real es `subtotal`) — esa rama nunca se ejecutaba, siempre caía al fallback de `precioTransportistaExterno` crudo. Ahora lee `lv.subtotal` correctamente.
- **Cero cambios necesarios** en `arca-iva.util.ts`, `cvlp-conceptos.util.ts`, `liquidacion-pdf.service.ts`, `update()` (recalcular comisión/IVA) ni `emitirLiquidacion()`: todos trabajan sobre el agregado `Liquidacion.bruto` ya persistido (neteado desde la creación) — verificado leyendo `buildCvlpConceptosList`/`computeLiquidacionTotales`, que arman el CVLP con una sola línea "Fletes" = `bruto`, nunca iteran `LiquidacionViaje.subtotal` para el cálculo fiscal (solo para descripciones de conceptos puntuales).
...
**Ya NO hay exclusión mutua con Liquidación ARCA/CVLP** — un viaje con IVA incluido se puede liquidar sin problema, el neteo lo resuelve. El guard que existía en `createLiquidacion` (`BadRequestException` listando viajes) se eliminó.

**Lock angosto, distinto al resto de `CAMPOS_FISCALES_VIAJE`**: `precioTransportistaIvaIncluidoPct` **NO** está en `CAMPOS_FISCALES_VIAJE` — tiene su propio guard en `update()`, bloqueado **solo** por `liquidacionEstado` (no por `facturacionEstado`). Es intencional: facturación al cliente y liquidación al transportista son ejes independientes, y bloquear este campo por una factura al cliente (sin relación con el transportista) dejaría el campo atascado para siempre en viajes viejos de un tenant que recién adopta `integracion-arca`. Una vez que el viaje tiene liquidación vigente, el % queda fijo porque `Liquidacion.bruto` ya se calculó neteando con ese valor — cambiarlo después desincronizaría el número ya persistido. Si se agrega un campo nuevo relacionado solo al transportista (no al cliente), evaluar si también necesita este lock angosto en vez de sumarlo sin más a `CAMPOS_FISCALES_VIAJE`.

**Frontend**: el checkbox se reemplazó por un input numérico "% de IVA ya incluido en el precio" (`ViajeCreatePage.tsx`, `ViajeEditModal.tsx`; `0` o vacío = no incluye), bloqueado solo cuando `liquidacionVigente` (mismo criterio de arriba). Ya no hace falta ningún patrón de advertencia por módulo ARCA — el `hasIntegracionArca`/`canAccessIntegracionArca` que se había agregado para la v1 (checkbox) se sacó por completo, junto con el prop `ViajeEditModal.hasIntegracionArca`.

**Migraciones**: `20260818120000_viaje_precio_transportista_incluye_iva` (v1, boolean) seguida de `20260819120000_viaje_precio_transportista_iva_incluido_pct` (dropea el boolean, agrega el float) — sin dato de producción en juego, la v1 nunca llegó a producción.
