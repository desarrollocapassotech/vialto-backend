...
export class PautService {
  ...
  // 1. Agregamos emisorId como parámetro opcional
  async generate(
    viajeId: string,
    tenantId: string,
    emisorId?: string,
    ...
    // --- NUEVO: INTERCEPTOR DE EMISOR ---
    // Si se pasa un emisorId distinto al contratante, buscamos sus datos en la DB
    if (emisorId && emisorId !== viaje.transportista?.id) {
      const emisor = await this.prisma.transportista.findFirst({
        // Nota: Si tu tabla transportista tiene `tenantId`, cambialo a: where: { id: emisorId, tenantId }
        where: { id: emisorId },
        select: {
          id: true,
          nombre: true,
      ...
      if (!emisor) {
        throw new NotFoundException(
          "El transportista seleccionado para emitir la nómina no existe.",
        );
      }

      // Reemplazamos en memoria: a partir de acá todo el sistema cree que este es el transportista del viaje.
      viaje.transportista = emisor;
    }
    // ------------------------------------

    if (
      !viajePuedeExportarPaut({
        transportistaId: viaje.transportista?.id ?? null,
      })
    ) {
    ...
    if (!viaje.transportista) {
      ...
      if (!viaje.transportista.permisoInternacional?.trim())
        t.push("Permiso Internacional");
      if (!viaje.transportista.fechaVencimientoPermiso)
        t.push("Vencimiento del Permiso Internacional");
      if (!viaje.transportista.domicilio?.trim()) t.push("Domicilio");
      if (!viaje.transportista.pais?.trim()) t.push("País");

      // Al haber reemplazado viaje.transportista, entityId ahora es el emisorId
      // Esto hace que el modal de "Datos Faltantes" guarde la información en la empresa correcta.
      if (t.length > 0)
        missingGroups["Transportista"] = {
          fields: t,
          entityId: viaje.transportista.id,
        };
    }
    ...
    if (!viaje.chofer) {
      ...
      if (c.length > 0)
        missingGroups["Chofer"] = { fields: c, entityId: viaje.chofer.id };
    }

    if (viajeFields.length > 0)
      missingGroups["Viaje"] = { fields: viajeFields, entityId: viajeId };
  ...
  // --- El resto de los métodos privados (resolvePautVehiculos, drawPaut, etc) se mantienen EXACTAMENTE iguales ---
  private resolvePautVehiculos(vehiculosViaje: PautViaje["vehiculosViaje"]): {
    ...
    return {
      ...
      semi:
        semi ??
        (cabeza ? (vehicles.find((v) => v.id !== cabeza.id) ?? null) : null),
    };
  }
  ...
  private dualInfoRowHeight(
    ...
    const leftH = this.measureFieldBlockHeight(
      doc,
      half,
      left.label,
      left.value,
    ...
    const rightH = this.measureFieldBlockHeight(
      doc,
      half,
      right.label,
      right.value,
  ...
  private drawPaut(
    ...
    > = [
      [
        ...
        {
          label: "PERMISO INTERNACIONAL:",
          value: t?.permisoInternacional ?? "",
        },
      ],
      [
        { label: "PAUT:", value: t?.paut ?? "" },
        {
          ...
          value: this.fmtDate(t?.fechaVencimientoPermiso),
        },
      ],
      [
    ...
    doc.text("CONCEPTO", x0 + hdrPad, hdrY, {
      width: C0 - hdrPad * 2,
      align: "center",
      lineBreak: false,
    });
    doc.text("CAMIÓN", x0 + C0 + hdrPad, hdrY, {
      width: C1 - hdrPad * 2,
      align: "center",
      lineBreak: false,
    });
    ...
    const vRows: Array<{ label: string; v1: string; v2: string }> = [
      ...
      {
        label: "AÑO",
        v1: camion?.anio ? String(camion.anio) : "",
        v2: semi?.anio ? String(semi.anio) : "",
      },
      {
        label: "CHASIS NRO",
        v1: camion?.nroChasis ?? "",
    ...
    choferRows.forEach(([label, value], i) => {
      const rowH = Math.max(
        PAUT.rowMinH,
        this.measureFieldBlockHeight(doc, W, label, value),
      );
